...
After cloning/downloading the repository, simply install walking-habits as a package into your target virtual environment:

    $ pip install PATH_TO_walking_habits
...
changes to the source code take immediate effect on the installed package.
    ...
    $ pip install -e PATH_TO_walking_habits
...
This script will also apply settings found in the module `walking_habits.prod_settings` (or a custom Python file supplied
with the `--settings` flag) and which takes precedence over the same settings
found in `walking_habits.settings`.
...
`run-walking_habits-prod` script to take effect even if you
...
for example) with the `walking_habits.wsgi` entry point
(defined in `wsgi.py`) like so:

    $ gunicorn walking_habits.wsgi
