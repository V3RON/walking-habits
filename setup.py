...
URL = "None"
AUTHOR = "Szymon Chmal"
AUTHOR_EMAIL = "szymonchmal@gmail.com"
LICENSE = "MIT license"
...
if not VERSION:
    with open(
        os.path.join(here, "src", "walking_habits", "__version__.py")
    ) as f:
        exec(f.read(), about)
else:
...
setup(
    ...
    scripts=["bin/run-walking_habits-prod"],
    entry_points={
        "console_scripts": [
            "run-walking_habits-dev=walking_habits.dev_cli:main"
        ]
    },
)
